    ...
    UBYTE *palshift)
{
   ...
   {
   case 1:
      dir = glb_ds1edit.new_dir1[user_dir];
      break;
   case 4:
      dir = glb_ds1edit.new_dir4[user_dir];
      break;
   case 8:
      dir = glb_ds1edit.new_dir8[user_dir];
      break;
   case 16:
      dir = glb_ds1edit.new_dir16[user_dir];
      break;
   case 32:
      dir = glb_ds1edit.new_dir32[user_dir];
      break;
   }
   ...
   for (i = 0; i < dcc->header.frames_per_dir; i++)
   {
      ...
      if (palshift)
      {
         for (y = 0; y < h; y++)
            for (x = 0; x < w; x++)
               putpixel(lay->bmp[i], x, y, palette_color(palshift[color_to_palette_index(getpixel(dcc->frame[dir][i].bmp, x, y))]));
      }
      else
