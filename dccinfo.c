...
// Define global variables that are declared as extern in dccinfo.h
char dcc_error[512];
...
int dcc_save_frame(DCC_S *dcc, int d, int f, BITMAP *frm_bmp)
{
...
#ifdef DCCINFO_SAVE_PCX
   {
      ...
      if (in != NULL)
      {
         ...
         fclose(in);
         sprintf(tmp, "tests\\images\\d%02i-f%03i.png", d, f);
         // Use Allegro 5.2.10 native PNG support
         save_png_a4_compat(tmp, frm_bmp, tmppal);
      }
      else
         return 1;
   }
            ...
            else
            {
               ...
               for (y = 0; y < cell->h; y++)
               {
                  for (x = 0; x < cell->w; x++)
                  {
                     ...
                     putpixel(cell->bmp, x, y, palette_color(pbe->val[pix]));
                  }
               }
            }
