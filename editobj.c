   ...
   for (loop = 0; loop < 3; loop++)
   {
      ...
      for (o_idx = 0; o_idx < glb_ds1[ds1_idx].obj_num; o_idx++)
      {
         ...
         if (is_shadow)
         {
            ...
            color1 = palette_color(186); // background label
            color2 = palette_color(209); // sub-tile & line color
            color3 = palette_color(214); // label border color
         ...
         if (is_shadow == FALSE)
         {
            color1 = palette_color(0);   // background label
            color2 = palette_color(255); // sub-tile & line color
            color3 = palette_color(255); // label border color
            if (IS_SELECTED(f1))
            {
               color1 = palette_color(118);
               color2 = palette_color(132);
               color3 = palette_color(132);
            }
            else if (IS_MOUSE_OVER(f1))
            {
               color2 = palette_color(129);
               if (IS_SELECTED(f2))
               {
                  color1 = palette_color(118);
                  color3 = palette_color(132);
         ...
         case 1:
            // draw shadow lines
            line(glb_ds1edit.screen_buff, ox0 + 1, oy0 + 17, ox0 + 1, oy0 + 1, palette_color(0));
            line(glb_ds1edit.screen_buff, ox0 + 1, oy0 + 1, lx0 + 1, ly0 + 1, palette_color(0));
            line(glb_ds1edit.screen_buff, lx0 + 1, ly0 + 1, lx0 + 1, ly0 - 8, palette_color(0));
         ...
         case 2:
            ...
            y2 = y1 + 2;
            if (d == -1)
               textprintf(glb_ds1edit.screen_buff, font, x2 - 8, y2, palette_color(255), "??");
            else
            {
               x2 -= (8 * strlen(glb_ds1edit.obj_desc[d].desc) / 2);
               textprintf(glb_ds1edit.screen_buff, font, x2, y2, palette_color(255), "%s",
            ...
            x2 = x1 + 2;
            y2 += 8 + 2;
            textprintf(glb_ds1edit.screen_buff, font, x2, y2, is_shadow ? palette_color(255) : palette_color(111), "#%i", o);
            ...
            if (id <= -100)
               ...
               x2 = lx0 - 2 * 8 - 4;
            textprintf(glb_ds1edit.screen_buff, font, x2, y2, is_shadow ? palette_color(255) : palette_color(11), "%i", type);

            // Id
            x2 += 1 * 8;
            textprintf(glb_ds1edit.screen_buff, font, x2, y2, is_shadow ? palette_color(255) : palette_color(255), " %i", id);
            ...
            if (p < 10)
               ...
               x2 = x1 + w + 4 - 3 * 8;
            if (p == 0)
               textprintf(glb_ds1edit.screen_buff, font, x2, y2, is_shadow ? palette_color(255) : palette_color(208), "P-");
            else
               textprintf(glb_ds1edit.screen_buff, font, x2, y2, is_shadow ? palette_color(255) : palette_color(133), "P%i", p);
            break;
         }
      }
   }
...
void editobj_prepare_edit_obj_win(int ds1_idx, int obj_idx)
{
   ...
   {
   case 1:
      SET_SELECTED(ptr_w->button[EB_ACT1].flags);
      break;
   case 2:
      SET_SELECTED(ptr_w->button[EB_ACT2].flags);
      break;
   case 3:
      SET_SELECTED(ptr_w->button[EB_ACT3].flags);
      break;
   case 4:
      SET_SELECTED(ptr_w->button[EB_ACT4].flags);
      break;
   case 5:
      SET_SELECTED(ptr_w->button[EB_ACT5].flags);
      break;
   }
